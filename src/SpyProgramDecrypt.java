...
public class SpyProgramDecrypt {
	...
	public static String decrypt(String enc) {
		...
		// this loop finds the first number (the multiplier), and remvoes all the random
		// leters
		for (int l = 0; l <= len; l++) {
		...
		// loop to check all the ascii values and convernt the string of numbers back
		// into words
		for (int h = 0; h < asLen; h++) {
			// variable to receive the letter from the ascii value
			char let = ' ';

			// declaring int variables, value is the ascii value for the char (+52 due to
			// shift), check is used to find if the ascii is 2 or 3 digits
			int value = 0, check = 0;
			...
			if (check == 1) {
				// this adds the following two digits, but not mathematically just after each
				// other
				value = Integer.parseInt(String.valueOf(stringAsc.charAt(h)) + String.valueOf(stringAsc.charAt(h + 1))
						+ String.valueOf(stringAsc.charAt(h + 2)));
			...
			// the rest of the ascii vals would be 2 digits
				...
				// since the ascii value is 2 digits, we add the current and next value, but
				// after each other not matjematically
				value = Integer.parseInt(String.valueOf(stringAsc.charAt(h)) + String.valueOf(stringAsc.charAt(h + 1)));
				h += 1;
			}
